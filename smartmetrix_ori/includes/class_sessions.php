...
class Sessions{
	...
	/*public static function login($email, $password, $mysqli) {
    	...
    	if ($stmt = $mysqli->prepare("SELECT smt_idmember, smt_username, smt_password, smt_salt FROM members WHERE smt_email = ? AND smt_idstatus = 'A' LIMIT 1 ")) {
        	...
        	if ($stmt->num_rows == 1) {
            	...
            	if (self::checkbrute($user_id, $mysqli) == true) {
                	...
                	if ($db_password == $password) {
                    	...
                    	$now = time();
                    	$mysqli->query("INSERT INTO login_attempts(smt_idmember, time)
                    	                	VALUES ('$user_id', '$now')");
                	    return false;
                	}
            	}
	...
	public static function login_check($mysqli) {
    	...
    	if (isset($_SESSION['user_id'], 
        		...
        		if ($stmt = $mysqli->prepare("SELECT smt_password 
                                      			FROM members 
                                      				WHERE smt_idmember = ? LIMIT 1")) {
