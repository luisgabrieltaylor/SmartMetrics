...
<head>
	...
	<br>
		...
		<nav>
			<ul>
				<li><a href="#" data-toggle="modal" data-target="#uploadPhoto">Subir Foto</a></li>
				<li><a href="#" data-toggle="modal" data-target="#aboutDialog">Acerca de</a></li>
			</ul>
		</nav><!-- / nav -->
	...
	<div id="columns">
		<?php
			$sql = "SELECT * FROM jv_photos WHERE jv_Status = 'N' ORDER BY jv_DateTime DESC";
			$resp = mysql_query($sql, $link);
			while($row = mysql_fetch_array($resp)){
				echo "<div class='pin' id='pin-".$row[0]."'>";
				echo "<img src='../".$row[1]."' />";
        echo "<p>Si <input name='status' type='radio' value='".$row[0].",Si' /> No <input name='status' type='radio' value='".$row[0].",No' /></p>";
  ...
  <div class="modal-dialog" role="document">
	...
	<script type="text/javascript">
       ...
       $("input[name=status]").click(function () {
          var status = $(this).val();
          $.get( "updatestatus.php", {estado: status},function(data){
